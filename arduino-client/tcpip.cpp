...
static uint16_t www_client_internal_datafill_cb(uint8_t fd) {
    ...
    if (fd==www_fd) {
    	const char* ahl = client_additionalheaderline;
            bfill.emit_p(PSTR("$S"), client_postval);
    }
...
void EtherCard::sendTCP(const char *hoststr, const char *data, void (*callback)(uint8_t,uint16_t,uint16_t), unsigned port) {
    client_hoststr = hoststr;
    client_postval = data;
    client_browser_cb = callback;
    www_fd = clientTcpReq(&www_client_internal_result_cb, &www_client_internal_datafill_cb, port);
}
